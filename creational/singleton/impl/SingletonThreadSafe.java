...
class DbSingleton {
    ...
    private DbSingleton() throws RuntimeException {
        // Java Reflection API can be used to bypass private property of this constructor, and instantiate.
        if (dbSingleton != null) {
    ...
    public static DbSingleton getInstance() { // static imp.
        // to make this function thread safe, we can make entire function synchronized
        // -> low peformance as mostly this function will be called to fetch the instance and not create

        if (dbSingleton == null) {
            // qq: Why monitor on Class: 
            // ans: 1) There isn't any instance in a static context. 2) Prevents parallel access of static members.
            // Fyi, if there exist a static instance, then it's fields & fns can still be accessed during this lock period (Recall: Java Locks)
            synchronized (DbSingleton.class) { 
                // what if 'x' threads enter dbSingleton == null and stop at synchronized
                // -> all will instantiate, and dbSingelton gets extra x-1 re-assignment
                // -> hence, if (dbSingleton == null) check is required.
                if (dbSingleton == null)
